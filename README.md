...
| **OpenAI** | GPT-4o, GPT-4o-mini, o1, o3, o3-mini, o4-mini | OpenAI API Key |
| **Anthropic** | Claude Opus 4, Claude Sonnet 4, Claude 3.5 Sonnet | Anthropic API Key |
...
CluadeX features a Claude-level agentic system with multi-step reasoning, self-correction, and automatic tool selection.

CluadeX มีระบบ Agent ระดับ Claude ที่คิดหลายขั้นตอน แก้ไขตัวเอง และเลือกใช้เครื่องมือโดยอัตโนมัติ
...
| **Explore Before Edit** | Always reads relevant files before editing — never guesses at existing code. | อ่านไฟล์ที่เกี่ยวข้องก่อนแก้เสมอ ไม่เดาโค้ดที่มีอยู่ |
| **Tool Chaining** | Chains tools together: read → edit → verify → test in a single flow. | ใช้เครื่องมือต่อเนื่อง: อ่าน → แก้ → ตรวจสอบ → ทดสอบ ในครั้งเดียว |
| **Smart Context** | Auto-loads referenced files, compacts history when context gets full. | โหลดไฟล์ที่อ้างถึงอัตโนมัติ บีบอัดประวัติเมื่อ context เต็ม |
| **Code Validation** | Validates code blocks for syntax errors before final response. Re-generates if issues found. | ตรวจสอบโค้ดก่อนส่ง ถ้ามีปัญหาจะสร้างใหม่ |
| **Auto Retry** | Automatic retry with exponential backoff on transient failures (429, 503, timeouts). | ลองใหม่อัตโนมัติเมื่อเกิด error ชั่วคราว (rate limit, timeout) |
| **Bilingual Prompts** | System prompt dynamically generated in Thai or English based on user's language setting. | System prompt สร้างแบบ dynamic ตามภาษาที่ผู้ใช้เลือก |
| **Feature-Gated Tools** | Tools respect feature toggles — disabled features are hidden from AI and blocked at execution. | เครื่องมือตรวจสอบ feature toggles — ฟีเจอร์ที่ปิดจะถูกซ่อนจาก AI |

---

### Agent Tools / เครื่องมือ Agent (25 เครื่องมือ)
...
| `search_content` | Search text in file contents (grep) | ค้นหาข้อความในเนื้อหาไฟล์ |
...
│   ├── ActivationService    # Key validation, tier gating, feature unlock
│   ├── AgentToolService     # 25 tools, permission checks, feature-gated execution
│   ├── BuddyService         # Companion generation (FNV-1a + Mulberry32 PRNG)
│   ├── CodeAgentService     # Agentic loop, bilingual prompts, retry, code review
│   ├── CodeExecutionService # Sandboxed code execution
│   ├── ContextMemoryService # Token counting, auto-compaction
│   ├── FileSystemService    # Safe file operations with path traversal protection
│   ├── GitService           # Git operations with input validation
│   ├── GitHubService        # GitHub CLI integration
│   ├── LocalizationService  # Thai/English i18n (170+ translations)
│   ├── PermissionService    # Allow/Deny/Ask permission rules
│   ├── PluginService        # Plugin discovery, catalog (20 plugins), management
...
├── App.xaml.cs           # DI container (20 services, 8 ViewModels)
├── MainWindow.xaml       # Sidebar, buddy widget, navigation, 7 pages
